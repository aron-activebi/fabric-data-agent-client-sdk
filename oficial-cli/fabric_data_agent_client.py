...
of the Fabric environment using Azure AD authentication.
...
2. Run the script - it will print a device-code login URL/code by default
...
class FabricDataAgentClient:
    ...
    This client handles:
    - Azure AD authentication with device-code or interactive browser flow
    - Automatic token refresh
    - Bearer token management for API calls
    - Proper cleanup of resources
    """
    
    def __init__(self, tenant_id: str, data_agent_url: str, auth_mode: str = None):
        """
        ...
        Args:
            ...
            auth_mode (str): "device_code" for terminal login or "browser" for local browser login
        """
        self.tenant_id = tenant_id
        self.data_agent_url = data_agent_url
        self.auth_mode = (auth_mode or os.getenv("FABRIC_AUTH_MODE") or "device_code").strip().lower()
    ...
    def _authenticate(self):
        """
        Perform Azure AD authentication and get initial token.
        """
        try:
            ...
            elif self.auth_mode == "device_code":
                print("Use the device-code URL/code shown below to sign in.")
                self.credential = DeviceCodeCredential(
                    tenant_id=self.tenant_id,
                    prompt_callback=self._print_device_code_message,
                )
            else:
                raise ValueError(
                    "Invalid auth_mode. Use 'device_code' or 'browser'. "
                    "You can also set FABRIC_AUTH_MODE in .env."
                )
    ...
    def _print_device_code_message(self, verification_uri, user_code, expires_on):
        """
        Print the Azure device-code login instruction in a terminal-friendly way.
        """
        if isinstance(expires_on, datetime):
            expires_at = expires_on.strftime("%Y-%m-%d %H:%M:%S")
        else:
            expires_at = time.ctime(expires_on)

        print("\nDevice code authentication")
        print(f"1. Open: {verification_uri}")
        print(f"2. Enter code: {user_code}")
        print(f"3. Complete sign-in before: {expires_at}")
